...
class photo_control extends admin_control {
	// 内容管理
	public function index() {
		// hook admin_photo_control_index_before.php
		...
		// 获取分类下拉框
		$cidhtml = $this->category->get_cidhtml_by_mid(4, $cid, '所有图集');
		...
		$cms_photo_arr = $this->cms_content->list_arr($where, 'id', -1, ($page-1)*$pagenum, $pagenum, $total);
		foreach($cms_photo_arr as &$v) {
		...
		$this->assign('cms_photo_arr', $cms_photo_arr);

		// hook admin_photo_control_index_after.php
	...
	// 发布图集
	public function add() {
		// hook admin_photo_control_add_before.php

		$uid = $this->_user['uid'];
		if(empty($_POST)) {
			$this->_pkey = 'content';
			$this->_ukey = 'photo-add';
			$this->_title = '发布图集';
			$this->_place = '内容 &#187; 内容管理 &#187 发布图集';

			$habits = (array)$this->kv->get('user_habits_uid_'.$uid);
			$cid = isset($habits['last_add_cid']) ? (int)$habits['last_add_cid'] : 0;

			$data = $this->kv->get('auto_save_photo_uid_'.$uid);
			if($data) {
			...
			if(!$this->cms_content_data->set($id, array('id' => $id, 'content' => $contentstr, 'images' => json_encode($images)))) {
				E(1, '写入内容数据表出错');
			}
			...
			$data = $this->kv->delete('auto_save_photo_uid_'.$uid);
			...
			// hook admin_photo_control_add_after.php
	...
	public function edit() {
		// hook admin_photo_control_edit_before.php
		...
		if(empty($_POST)) {
			...
			$this->_pkey = 'content';
			$this->_ukey = 'photo-edit-cid-'.$cid.'-id-'.$id;
			$this->_title = '编辑图集';
			$this->_place = '内容 &#187; 内容管理 &#187 编辑图集';
			...
			// hook admin_photo_control_edit_after.php
	...
	// 删除图集
	public function del() {
		// hook admin_photo_control_del_before.php
		...
		// hook admin_photo_control_del_after.php

		$err = $this->cms_content->xdelete('photo', $id, $cid);
		if($err) {
			E(1, $err);
		}else{
			E(0, '删除成功！');
	...
	public function batch_del() {
		// hook admin_photo_control_batch_del_before.php

		$id_arr = R('id_arr', 'P');
		if(!empty($id_arr) && is_array($id_arr)) {
			$err_num = 0;
			foreach($id_arr as $v) {
				$err = $this->cms_content->xdelete('photo', $v[0], $v[1]);
				if($err) $err_num++;
			}
	...
	public function auto_save() {
		$this->kv->set('auto_save_photo_uid_'.$this->_user['uid'], $_POST) ? E(0, '自动保存成功！') : E(1, '自动保存失败！');
	}
	...
	// hook admin_photo_control_after.php
}
